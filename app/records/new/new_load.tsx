...
import { addAnimalRecord, addLoadRecord } from '../../../firebase/firestore';
...
interface FormData {
  ...
  category: string;
  gender: 'male' | 'female';
  status: 'active' | 'loaded out' | 'deceased';
  description: string;
  ownerDetails: string;
  loadOutDate: Date;
  // loadOutPrice: string;
...
export default function NewRecordScreen(): React.ReactElement {
  ...
  const handleSubmit = async (): Promise<void> => {
    ...
    try {
      ...
      const totalLoad = parseFloat(formData.LoadInPrice) || 0;
      ...
      >
        <ScrollView style={styles.scrollView}>
          ...
          <View style={styles.formContainer}>
            ...
            <View style={styles.card}>
              <Text style={styles.cardTitle}>Load In Details</Text>
              
              <View style={styles.inputGroup}>
                {renderFieldLabel('Load In Date')}
                <TouchableOpacity
                ...
                >
                  ...
                  <Text style={styles.dateButtonText}>
                    {formData.LoadInDate.toLocaleDateString()}
                  </Text>
                </TouchableOpacity>
                {showPurchaseDatePicker && (
                  <DateTimePicker
                    ...
                    onChange={(event, date?: Date) => {
                      ...
                      if (date) {
                        setFormData(prev => ({ ...prev, LoadInDate: date }));
                      }
                    }}
                  />
                )}
              ...
              <View style={styles.inputGroup}>
                {renderFieldLabel('Load In Price')}
                <View style={styles.inputWithIcon}>
                  <Ionicons name="cash" size={20} color="#777" style={styles.inputIcon} />
                  <TextInput
                    style={styles.input}
                    value={formData.LoadInPrice}
                    onChangeText={(text: string) => setFormData(prev => ({ ...prev, LoadInPrice: text }))}
                    placeholder="Enter Load In price"
                    placeholderTextColor="#999"
            ...
            <View style={styles.card}>
              ...
              <View style={styles.inputGroup}>
                <View style={styles.statusContainer}>
                  {[
                    ...
                    { value: 'loaded out' as const, icon: 'cash', label: 'loaded out' },
              ...
              {formData.status === 'loaded out' && (
                <>
                  <View style={styles.inputGroup}>
                    {renderFieldLabel('Load out Date')}
                    <TouchableOpacity
                      style={styles.dateButton}
                    ...
                    >
                      <Ionicons name="calendar" size={20} color="#777" style={styles.dateButtonIcon} />
                      <Text style={styles.dateButtonText}>
                        {formData.loadOutDate.toLocaleDateString()}
                      </Text>
                    </TouchableOpacity>
                    {showSoldDatePicker && (
                      <DateTimePicker
                        ...
                        onChange={(event, date?: Date) => {
                          ...
                          if (date) {
                            setFormData(prev => ({ ...prev, loadOutDate: date }));
                          }
                        }}
                      />
                    )}
...
{/* 
                  <View style={styles.inputGroup}>
                    {renderFieldLabel('Load out price', true)}
                    <View style={styles.inputWithIcon}>
                      <Ionicons name="cash" size={20} color="#777" style={styles.inputIcon} />
                      <TextInput
                        style={styles.input}
                        value={formData.loadOutPrice}
                        onChangeText={(text: string) => setFormData(prev => ({ ...prev, loadOutPrice: text }))}
                        placeholder="Enter load out price"
                        placeholderTextColor="#999"
                        keyboardType="numeric"
                      />
                    </View>
            ...
            <View style={styles.card}>
              ...
              <View style={styles.inputGroup}>
                {renderFieldLabel('Owner Details')}
                <TextInput
                  style={[styles.input, styles.textArea]}
                  value={formData.ownerDetails}
                  onChangeText={(text: string) => setFormData(prev => ({ ...prev, ownerDetails: text }))}
